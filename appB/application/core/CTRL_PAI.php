...
class CTRL_PAI extends CI_Controller {
	...
	function listagem(){
        ...
        $config['total_rows'] = $this->db->query("select count(".$this->configuracao->chave.") total 
												  ...
												  where status = 1 $this->master_detalhe_filtro")->result()[0]->total;
		...
		foreach($this->configuracao->colunas as $c => $v ) :
			...
			elseif($v->tipo == 'file') :
				...
				if ( ! $this->upload->do_upload($c))
				{
					...
					$this->session->set_flashdata('message', msg_json('Erro ao fazer Upload: '.$error, 2) );
					$passa_upload = false;
				}else{
